...
begin
 ...
 if code='VERIFY_TEAM_ROSTER_MAPPINGS' then
  ...
  if expected_option_count>0 and verified_at<x.owner_deadline then
   perform public.raise_rollover_preflight_failure_phase3b6b(
...
-- Handler v2: the seven-day owner option window is required only when
-- the canonical plan contains at least one eligible owner option.
update public.rollover_execution_handler_registry
set
  handler_version = 2,
  input_schema_version = 'phase3b6b-option-window-input-v2'
where operation_code = 'VERIFY_OPTION_WINDOW_CLOSED'
  and execution_owner = 'execution';
...
begin
  if (
    ...
    where operation_code = 'VERIFY_OPTION_WINDOW_CLOSED'
      ...
      and input_schema_version = 'phase3b6b-option-window-input-v2'
      and enabled
  ) <> 1 then
    raise exception 'VERIFY_OPTION_WINDOW_CLOSED handler v2 registry update failed';
  end if;
end
$$;
