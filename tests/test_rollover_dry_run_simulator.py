...
def source(team_count=10,**kw):
 teams=tuple(team(i) for i in range(team_count));cap=CapAuthorityPlan("l",2025,2026,Decimal(227),Decimal(227),"hard",Decimal(227),Decimal(1),"nearest",Decimal(100*team_count),Decimal(0),Decimal(0),Decimal(0),Decimal(0),teams,(),(),"c","d")
...
class DryRunTests(unittest.TestCase):
 ...
 def test_authoritative_team_set_valid_at_supported_cardinalities(self):
  for count in (1,10,32,100,2000):
   with self.subTest(count=count):
    validation=RolloverDryRunValidator().validate(self.s.simulate(source(count),generated_at=self.now));self.assertTrue(validation.checks["team_result_set_matches_authority"]);self.assertTrue(validation.executable)
 def test_missing_duplicate_and_foreign_team_results_fail(self):
  result=self.s.simulate(source(10),generated_at=self.now)
  missing=replace(result,team_results=result.team_results[:-1]);self.assertFalse(RolloverDryRunValidator().validate(missing).checks["team_result_set_matches_authority"])
  duplicate=replace(result,team_results=result.team_results[:-1]+(result.team_results[0],));validation=RolloverDryRunValidator().validate(duplicate);self.assertFalse(validation.checks["team_results_unique"]);self.assertFalse(validation.executable)
  foreign=replace(result,team_results=result.team_results[:-1]+(replace(result.team_results[-1],league_team_id="foreign"),));self.assertFalse(RolloverDryRunValidator().validate(foreign).checks["team_result_set_matches_authority"])
